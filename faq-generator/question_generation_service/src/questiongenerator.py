...
class QuestionGenerator:
    ...
    def check_local_model_dir(self, path):
    ...
    def __init__(self) -> None:
        ...
        # path of the local models directory
        PATH = "./models/"
        ...
        if not self.check_local_model_dir(PATH):
            self.qg_tokenizer = AutoTokenizer.from_pretrained(
                QG_PRETRAINED, use_fast=False
            )
            self.qg_model = AutoModelForSeq2SeqLM.from_pretrained(QG_PRETRAINED)
            self.qg_tokenizer.save_pretrained(PATH)
            self.qg_model.save_pretrained(PATH)
        else:
            self.qg_tokenizer = AutoTokenizer.from_pretrained(PATH, use_fast=False)
            self.qg_model = AutoModelForSeq2SeqLM.from_pretrained(PATH)
    ...
    def generate(
        ...
        if use_evaluator:
            logger.debug("Evaluating QA pairs...\n")
            ...
            logger.debug("Skipping evaluation step.\n")
